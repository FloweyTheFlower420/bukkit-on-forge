package com.floweytf.forgebukkit.block.data;
...
public class ForgeBukkitBlockData implements BlockData {
    ...
    protected ForgeBukkitBlockData() {
        throw new AssertionError("Template Constructor");
    }

    protected ForgeBukkitBlockData(BlockState state) {
        this.state = state;
    }

    @Override
    public Material getMaterial() {
        return ForgeBukkitMagicNumbers.getMaterial(state.getBlock());
    }
    ...
    protected <B extends Enum<B>> Set<B> getValues(EnumProperty<?> mc, Class<B> bukkit) {
        ImmutableSet.Builder<B> values = ImmutableSet.builder();

        for (Enum<?> e : mc.getAllowedValues())
    ...
    public String toString(Map<Property<?>, Comparable<?>> states) {
        ...
        if (!states.isEmpty()) {
            stateString.append('[');
            stateString.append(states.entrySet().stream().map(BlockStateHolder.STATE_TO_VALUE).collect(Collectors.joining(",")));
            stateString.append(']');
        }
    ...
    public NBTTagCompound toStates() {
        ...
        for (Map.Entry<Property<?>, Comparable<?>> entry : state.getStateMap().entrySet()) {
            Property Property = (Property) entry.getKey();

            compound.setString(Property.getName(), Property.a(entry.getValue()));
        }
