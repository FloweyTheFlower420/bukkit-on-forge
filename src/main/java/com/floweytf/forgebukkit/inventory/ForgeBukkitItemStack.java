...
public final class ForgeBukkitItemStack extends ItemStack {

    public static net.minecraft.item.ItemStack toMcCopy(ItemStack original) {
        if (original instanceof ForgeBukkitItemStack) {
            ForgeBukkitItemStack stack = (ForgeBukkitItemStack) original;
    ...
    public static ItemStack fromMcCopy(net.minecraft.item.ItemStack original, int amount) {
    ...
    public static ForgeBukkitItemStack asCraftMirror(net.minecraft.item.ItemStack original) {
        return new ForgeBukkitItemStack((original == null || original.isEmpty()) ? null : original);
    }

    public static ForgeBukkitItemStack asCraftCopy(ItemStack original) {
        if (original instanceof ForgeBukkitItemStack) {
            ForgeBukkitItemStack stack = (ForgeBukkitItemStack) original;
            return new ForgeBukkitItemStack(stack.handle == null ? null : stack.handle.cloneItemStack());
        }
        return new ForgeBukkitItemStack(original);
    }

    public static ForgeBukkitItemStack asNewCraftStack(Item item) {
     ...
     * Mirror
     */
    private ForgeBukkitItemStack(net.minecraft.item.ItemStack item) {
        this.handle = item;
    }

    private ForgeBukkitItemStack(ItemStack item) {
        this(item.getType(), item.getAmount(), item.getDurability(), item.hasItemMeta() ? item.getItemMeta() : null);
    }

    private ForgeBukkitItemStack(Material type, int amount, short durability, ItemMeta itemMeta) {
        setType(type);
        setAmount(amount);
    ...
    static Map<Enchantment, Integer> getEnchantments(net.minecraft.item.ItemStack item) {
        ListNBT list = (item != null && item.hasEnchantments()) ? item.getEnchantments() : null;
        ...
        for (int i = 0; i < list.size(); i++) {
            ...
            Enchantment enchant = Enchantment.getByKey(ForgeBukkitNamespacedKey.fromStringOrNull(id));
            if (enchant != null) {
    ...
    @Override
    public ForgeBukkitItemStack clone() {
        ForgeBukkitItemStack itemStack = (ForgeBukkitItemStack) super.clone();
        if (this.handle != null) {
    ...
    public boolean isSimilar(ItemStack stack) {
        ...
        if (!(stack instanceof ForgeBukkitItemStack)) {
            return stack.getClass() == ItemStack.class && stack.isSimilar(this);
        }

        ForgeBukkitItemStack that = (ForgeBukkitItemStack) stack;
